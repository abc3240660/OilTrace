...
int main(void)
{
	...
	if (1) {
		...
		if ((iapf0 == 0x1A1A) && (iapf1 == 0x2B2B)) {// BAK分区有新固件标志,需要执行BAK到APP的拷贝			
			for (i=0; i<100; i++) {// 不超过200K程序
				for (j=0; j<1024; j++) {//u8变成u16后的字节数据					   
					IAPBUF[j] = *(u16*)(SYS_APP_BAK_SAVE_ADDR_BASE+k+i*2048); 	//将临时Flash里的数据读出来	 每次读1K字节
					k += 2;
				}
				k = 0;
				FLASH_Unlock();
				FLASH_ClearFlag(FLASH_FLAG_EOP|FLASH_FLAG_PGERR|FLASH_FLAG_WRPRTERR);
				FLASH_ErasePage(SYS_APP_SAVE_ADDR_BASE+i*2048);// 先擦后写，每次擦除一页地址(1K);
				for (j=0; j<1024; j++) {// u8变成u16后的字节数据					   
					FLASH_ProgramHalfWord((SYS_APP_SAVE_ADDR_BASE+j*2+i*2048), IAPBUF[j]);// flash为一个字节存储，16位数据必须地址加2
				}
				FLASH_Lock();
				j = 0;
				delay_ms(2);
